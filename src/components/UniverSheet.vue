...
const init = (data = {}) => {
  ...
  univerAPI.onCommandExecuted((command) => {
    if (command.id === 'sheet.operation.set-cell-edit-visible' && command.params && command.params.visible) {
  ...
  univerAPI.onCommandExecuted((command) => {
    if (command.id === 'sheet.operation.set-cell-edit-visible' && command.params && !command.params.visible) {
      ...
      const snapshot = activeWorkbook && activeWorkbook.getSnapshot()
      console.log(snapshot)

    }
  })
  const activeWorkbook = univerAPI.getActiveWorkbook();
  activeWorkbook && activeWorkbook.onSelectionChange((selection) => {
